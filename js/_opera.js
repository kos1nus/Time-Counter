function GetAbout(){return 'Ver: '+widget.version+' || <b>'+widget.author+'</b> || '+widget.authorEmail;}
function OpenNewTab(value){opera.extension.postMessage({action:'new_tab',value:value});}
function i18n(name){
	var msg;
...
function IndexFload(){
	opera.extension.windows.onblur = function(){er('Главное окно НЕ активно'); FuncCT();}
	...
	opera.extension.onconnect = function(e){
		e.source.postMessage({action:'change_idle_time',value:300000});
		TabOnFocus();
	}
	opera.extension.onmessage = function(e){
		switch(e.data.action){
			case 'new_tab':opera.extension.tabs.create({url:e.data.value,focused:true});break;
			case 'idle_stop':FuncCT();break;
			case 'idle_start':TabOnFocus();break;
		}
	} 
